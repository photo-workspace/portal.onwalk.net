...
const formatImageTitle = (value?: string) =>
...
export default function ImagesGallery({
  ...
  const totalPages =
    externalTotalPages ?? Math.max(1, Math.ceil(totalImages / PAGE_SIZE));
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white/80 p-6 shadow-[0_18px_40px_-32px_rgba(15,23,42,0.35)] backdrop-blur">
          {currentItems.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center text-sm text-slate-500">
              暂无图片，请将图片上传到 R2 的 images/ 目录下。若需使用本地
              public/images/ 调试，请设置 ENABLE_LOCAL_MEDIA_FALLBACK=true。
            </div>
          ) : (
